//This file CANNOT be compiled alone, it should be placed in the sst-rs project under sst-rs/sst-benchmarks/src/bin/
// and then built with in release mode using cargo --build --bin gen_queries --release
// this results in a binary in sst-rs/sst-benchmarks/target/release/ which can be placed in the same folder as "gen_connec_data.py" which is then able to 
// generate query data using this file.

// based (heavily) on the 'sst-rs/sst-benchmarks/src/bin/bench_queries.rs' file from the sst-rs project.
