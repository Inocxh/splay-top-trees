...
TEST_CASE( "Rotation on path", "[rotations]") {
    int size = 4;
    Tree tree = Tree(size);
    Edge<>* node = tree.add_edge(0, 1, None());
    Edge<>* sibling = tree.add_edge(1, 2, None());
    Edge<>* uncle = tree.add_edge(3, 2, None());
    
    LeafNode<>* n = new LeafNode<>(node, 1, false);
    LeafNode<>* s = new LeafNode<>(sibling, 2, false);
    LeafNode<>* u = new LeafNode<>(uncle, 1, true);
    
    InternalNode<>* parent = new InternalNode<>(1, false);
    parent->set_children(n, s);
    n->set_parent(parent);
    s->set_parent(parent);

    InternalNode<>* grandparent = new InternalNode<>(0, false);
    ...
    SECTION("Standard") {
        ...
        REQUIRE(grandparent->get_child(0) == n);
        REQUIRE(grandparent->get_child(1) == parent);
        REQUIRE(parent->get_child(0) == s);
        REQUIRE(parent->get_child(1) == u);
    ...
    SECTION("Flipped") {
        ...
        REQUIRE(grandparent->get_child(0) == parent);
        REQUIRE(grandparent->get_child(1) == n);
        REQUIRE(parent->get_child(0) == u);
        REQUIRE(parent->get_child(1) == s);
...
TEST_CASE( "Rotation on star same side", "[rotations]") {
    ...
    Edge<>* node = tree.add_edge(0, 1, None());
    Edge<>* sibling = tree.add_edge(1, 2,None());
    Edge<>* uncle = tree.add_edge(1, 3,None());
    
    LeafNode<>* n = new LeafNode<>(node, 1, false);
    LeafNode<>* s = new LeafNode<>(sibling, 1, false);
    LeafNode<>* u = new LeafNode<>(uncle, 1, false);
...
TEST_CASE("semi_splay_step on path (Case 1 and 3)", "[semi_splay_step]") {
    ...
    Edge<>* node = tree.add_edge(0, 1,None());
    Edge<>* sibling = tree.add_edge(1, 2,None());
    Edge<>* uncle = tree.add_edge(2, 3,None());
    Edge<>* granduncle = tree.add_edge(3, 4,None());

    //Top tree setup
    LeafNode<>* top_node = new LeafNode<>(node, 1, false);
    LeafNode<>* top_sibling = new LeafNode<>(sibling, 2, false);
    LeafNode<>* top_uncle = new LeafNode<>(uncle, 2, false);
    LeafNode<>* top_granduncle = new LeafNode<>(granduncle, 1, false);
    
    InternalNode<>* top_parent = new InternalNode<>(1, false);
    top_parent->set_children(top_node, top_sibling);
    top_sibling->set_parent(top_parent);
    top_node->set_parent(top_parent);

    InternalNode<>* top_grandparent = new InternalNode<>(1, false);
    ...
    InternalNode<>* top_ggparent = new InternalNode<>(0, false);
    ...
    InternalNode<>* res = (InternalNode<>*) (top_node->semi_splay_step());;
    ...
    REQUIRE((
        top_parent->get_child(0) == top_sibling &&
        top_parent->get_child(1) == top_uncle && 
    ...
    REQUIRE((
        top_grandparent->get_child(0) == top_node &&
        top_grandparent->get_child(1) == top_parent &&
    ...
    REQUIRE((
        top_ggparent->get_child(0) == top_grandparent &&
        top_ggparent->get_child(1) == top_granduncle &&
...
TEST_CASE("Case 3", "[semi_splay_step]") {
    int size = 6;
    Tree tree = Tree(size);
    Edge<>* gguncle = tree.add_edge(0, 1,None());
    Edge<>* node = tree.add_edge(1, 2,None());
    Edge<>* sibling = tree.add_edge(2, 3,None());
    Edge<>* uncle = tree.add_edge(3, 4,None());
    Edge<>* granduncle = tree.add_edge(4, 5,None());
    ...
    LeafNode<>* top_uncle = new LeafNode<>(uncle, 2, false);
    LeafNode<>* top_granduncle = new LeafNode<>(granduncle, 1, false);
    LeafNode<>* top_gguncle = new LeafNode<>(gguncle, 1, false);
    
    InternalNode<>* top_parent = new InternalNode<>(2, false);
    top_parent->set_children(top_node, top_sibling);
    top_sibling->set_parent(top_parent);
    top_node->set_parent(top_parent);
    ...
    InternalNode<>* top_gggparent = new InternalNode<>(0, false);
    ...
    REQUIRE((
        top_gggparent->get_child(0) == top_ggparent &&
        top_gggparent->get_child(1) == top_gguncle &&
...
TEST_CASE("semi_splay_step on pronged tree (Case 2 and 4)", "[semi_splay_step]") {
    int size = 5;
    Tree tree = Tree(6);
    Edge<>* node = tree.add_edge(3, 1,None());
    Edge<>* sibling = tree.add_edge(0, 1,None());
    Edge<>* uncle = tree.add_edge(2, 1,None());
    Edge<>* granduncle = tree.add_edge(3, 4,None());
    ...
    SECTION("Case 4", "[semi_splay_step]") {
        ...
        REQUIRE(top_uncle->get_parent() == top_parent);
        REQUIRE((
            top_parent->get_child(0) == top_uncle &&
            top_parent->get_child(1) == top_sibling &&
            !top_parent->is_flipped()
        ));
        ...
        REQUIRE(top_parent->get_parent() == top_grandparent);
        REQUIRE((
            top_grandparent->get_child(0) == top_parent &&
            top_grandparent->get_child(1) == top_node &&
            !top_grandparent->is_flipped()
        ));
