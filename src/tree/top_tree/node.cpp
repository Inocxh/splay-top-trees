...
bool NodeBase::is_point() {
    return num_boundary_vertices < 2;
}

bool NodeBase::is_path() {
    return num_boundary_vertices == 2;
}
void NodeBase::flip() {
    this->flipped = !this->flipped;
}
InternalNodeBase* NodeBase::get_parent() {
    return this->parent;
}

void NodeBase::set_parent(InternalNodeBase* p) {
    this->parent = p;
}

NodeBase* NodeBase::get_sibling() {
    //The parent is *always* an InternalNodeBase
    InternalNodeBase* parent = this->get_parent();
    if (!parent) {
        return nullptr;
    }
...
bool NodeBase::is_left_child() {
    if (!this->parent) {
        return false;
    }
    return this == this->parent->children[parent->flipped];
}
//Assumes that the NodeBase can be part of a valid rotation.
void NodeBase::rotate_up() {
        InternalNodeBase* parent = this->get_parent();
        InternalNodeBase* grandparent = parent->get_parent();
        NodeBase* sibling = this->get_sibling();
        NodeBase* uncle = parent->get_sibling();
...
NodeBase* NodeBase::semi_splay_step() {
    NodeBase* NodeBase = this;

    while (true) {
        InternalNodeBase* parent = NodeBase->get_parent();
        if (!parent) {
            return nullptr;
        }
        InternalNodeBase* grandparent = parent->get_parent();
        ...
        // point, point, point
        // point, path,  point
        if (NodeBase->is_point() && grandparent->is_point()) {
            NodeBase->rotate_up();
            return grandparent;            
        }
        InternalNodeBase* ggparent = grandparent->get_parent();
        if (!ggparent) {
            return nullptr;
        }
        // b1,   b2,    b3
        ...
        if (parent->is_path() && (grandparent->is_path() || ggparent->is_point())) {
            ...
            if (NodeBase->is_left_child() == parent->is_left_child()) {
                NodeBase->rotate_up();
                return grandparent;
                ...
                NodeBase->get_sibling()->rotate_up();
                parent->rotate_up();
                return ggparent;
            }
        }
...
void NodeBase::full_splay() {
    while (true) {
        NodeBase* top = this->semi_splay_step();
        if (!top) { 
            return;
        }
        top->semi_splay_step();
